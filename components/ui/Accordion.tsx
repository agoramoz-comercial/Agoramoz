...
export function Accordion({
  ...
  return (
    <RA.Root type="single" collapsible className={cn('divide-y divide-[color:var(--border)] border-y border-[color:var(--border)]', className)}>
      {items.map((item, i) => (
        <RA.Item key={i} value={`item-${i}`} className="group">
          <RA.Header>
            <RA.Trigger className="flex w-full items-start justify-between gap-6 py-6 text-left">
