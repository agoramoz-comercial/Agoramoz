...
| T-15 | **IDOR no admin** — aceder a diagnóstico de outro registo por id | Fechada do lado da ESCRITA: nenhuma escrita directa em tabela a partir da aplicação. As 11 funções de 0006 verificam o papel, derivam o autor de `auth.uid()` e auditam na mesma transação; `diagnostics_update`, `deals_update`, `activities_insert` e `profiles_admin_write` foram removidas. Provado por violação tentada: `update` directo afecta 0 linhas. **Leitura é deliberadamente aberta a toda a equipa** (`is_staff()`), não é omissão: a equipa é pequena e um comercial precisa de ver o diagnóstico de qualquer colega. Se algum dia deixar de ser verdade, é uma política nova, não um remendo |
| T-16 | **CSRF nas acções administrativas** | Fechada — todas as mutações são Server Actions, que o Next verifica contra `Origin`/`Host`. Não existe caminho `GET` que altere estado; sair é um POST |
| T-17 | **XSS via respostas do cliente** renderizadas no documento e no admin | Fechada — `react/no-danger` é regra de ESLint. A única excepção é o JSON-LD, com `<` escapado como `\u003c`. O texto livre do cliente aparece em `<blockquote>`, citado como palavras dele |
| T-18 | **Escalada por chave de serviço** — `service_role` do Supabase ignora RLS | Fechada — a chave de serviço serve só a ingestão pública. O admin usa a sessão do utilizador, sujeita a RLS, e a chave anónima também fica do lado do servidor: o browser não recebe token nenhum |
