...
--
-- ORDEM IMPORTA. Este ficheiro LARGA a função de ingestão de 16 argumentos e
-- recria-a com 17. Entre o momento em que isto corre e o momento em que o
-- deployment novo entra no ar, o site em produção chama uma função que já não
-- existe — e o formulário devolve 503.
--
-- Por isso: avise-me assim que isto correr, e eu publico imediatamente. A
-- janela é de minutos. Não corra isto e vá almoçar.
--
-- É idempotente no que pode ser: `create or replace` nas funções, `drop ... if
-- exists`. Os `create table` NÃO são — se já existirem, dá erro, e o erro é a
-- informação (já foi aplicado).
-- ============================================================================
...
-- ============================================================================
-- VERIFICAÇÃO — correr depois, e confirmar que dá o que está à direita
-- ============================================================================

-- Esperado: 17
select count(*) as parametros_da_ingestao
from information_schema.parameters
where specific_schema = 'public'
  and specific_name = (
    select specific_name from information_schema.routines
    where routine_schema = 'public' and routine_name = 'ingest_diagnostic_response'
  );

-- Esperado: exactamente UMA linha. Duas significa que ficou uma sobrecarga, e
-- aí o PostgREST não sabe qual chamar.
select count(*) as versoes_da_funcao
from information_schema.routines
where routine_schema = 'public' and routine_name = 'ingest_diagnostic_response';

-- Esperado: 2 tabelas novas, ambas com RLS ligada.
select c.relname as tabela, c.relrowsecurity as rls_ligada
from pg_class c
join pg_namespace n on n.oid = c.relnamespace
where n.nspname = 'public'
  and c.relname in ('response_attribution', 'analytics_events')
order by 1;

-- Esperado: 2 colunas novas em deals.
select column_name, data_type
from information_schema.columns
where table_schema = 'public' and table_name = 'deals'
  and column_name in ('acquisition_channel', 'acquisition_campaign')
order by 1;

-- Esperado: 4 gatilhos (atribuição imutável, deal criada, deal ganha, documento visto).
select trigger_name, event_object_table
from information_schema.triggers
where trigger_schema = 'public'
  and trigger_name in ('deals_atribuicao_imutavel', 'deals_evento_criada',
                       'deals_evento_ganha', 'documents_evento_visto')
order by 1;

-- Esperado: a vista existe.
select table_name from information_schema.views
where table_schema = 'public' and table_name = 'funil_aquisicao';

-- O sanitizador do lado da base, provado por tentativa.
-- Esperado: google | (null) | (null) | (null) | (null)
select public.utm_limpo('GOOGLE')                        as deve_ser_google,
       public.utm_limpo('<script>alert(1)</script>')      as deve_ser_nulo_1,
       public.utm_limpo(repeat('a', 65))                  as deve_ser_nulo_2,
       public.utm_limpo('campanha,outra')                 as deve_ser_nulo_3,
       public.utm_limpo('null')                           as deve_ser_nulo_4;
